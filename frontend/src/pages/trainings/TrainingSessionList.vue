<template>
    <div class="p-4">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
            ...
            <Button label="Nouvelle session" icon="pi pi-plus" @click="openSessionDialog" />
        </div>
        ...
        <!-- Dialog Session -->
        <Dialog v-model:visible="sessionDialogVisible"
            :header="editingSession ? 'Modifier la session' : 'Nouvelle session'" :modal="true"
            ...
            <div class="field mt-4">
                ...
                <Dropdown id="organization" v-model="sessionForm.training_organization_id"
                    :options="store.organizations" optionLabel="name" optionValue="id"
                    placeholder="Sélectionnez un organisme" filter showClear />
            </div>
...
watch([() => sessionForm.value.training_id, () => sessionForm.value.start_date], ([newTrainingId, newStartDate]) => {
...
const saveSession = async () => {
    if (!sessionForm.value.training_id || !sessionForm.value.start_date || !sessionForm.value.end_date || !sessionForm.value.location) {
    ...
    try {
        if (editingSession.value) {
            ...
            toast.add({ severity: 'success', summary: 'Succès', detail: 'Session mise à jour', life: 3000 })
        } else {
