    ...
    <div class="p-4">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
            ...
            <div class="flex gap-2">
                <Button label="Nouvelle catégorie" icon="pi pi-folder-plus" severity="secondary" outlined
        ...
        <div v-else class="flex flex-col gap-8">
            <div v-for="category in categoriesWithTrainings" :key="category.id">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    <TrainingCard v-for="training in getTrainingsByCategory(category.id)" :key="training.id"
                        :training="training" @click="editTraining(training)" />
                    <!-- Add Card -->
                    <div class="border-2 border-dashed border-gray-300 rounded-lg flex flex-col items-center justify-center p-6 cursor-pointer hover:border-blue-500 hover:bg-blue-50 transition-colors min-h-[200px]"
...
const saveTraining = async () => {
    ...
    try {
        if (editingTraining.value) {
            ...
            toast.add({ severity: 'success', summary: 'Succès', detail: 'Formation mise à jour', life: 3000 })
        } else {
