<template>
    <div class="p-4" v-if="session">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
            ...
            <Button label="Modifier" icon="pi pi-pencil" severity="secondary" outlined @click="editSession" />
        </div>
                    ...
                    <template #content>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <span class="font-medium">{{ session.cost ? formatCurrency(session.cost) : '-' }}</span>
                            </div>
                        </div>
                    </template>
                </Card>
                ...
                <Card>
                    <template #title>
                        <div class="flex justify-between items-center">
                            <span>Documents</span>
                            <Button icon="pi pi-plus" text rounded size="small" v-tooltip="'Ajouter un document'" />
                        </div>
                    </template>
                    <template #content>
                        ...
                        <div class="flex flex-col gap-3">
                            ...
                            <div class="w-full bg-gray-100 rounded-full h-2">
                                <div class="bg-blue-500 h-2 rounded-full"
                                    :style="{ width: getParticipationPercentage() + '%' }"></div>
                            </div>
                        </div>
                    </template>
                </Card>
...
const session = computed(() => store.sessions.find(s => s.id === parseInt(route.params.id as string)))
...
const addParticipant = async (data: { user_id: number, status: string }) => {
    try {
        ...
        toast.add({ severity: 'error', summary: 'Erreur', detail: e.message || "Erreur lors de l'ajout", life: 3000 })
    }
}
...
const updateParticipant = async (data: any) => {
    try {
        ...
        toast.add({ severity: 'success', summary: 'Succès', detail: 'Participant mis à jour', life: 3000 })
    } catch (e: any) {
