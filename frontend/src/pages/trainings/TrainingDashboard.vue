    ...
    <div class="p-4">
        ...
        <div class="mb-8">
            ...
            <div v-if="renewalsNeeded.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <Card v-for="renewal in renewalsNeeded" :key="renewal.id" class="border-l-4"
                    :class="renewal.is_expired ? 'border-red-500 bg-red-50' : 'border-orange-500 bg-orange-50'">
                    <template #content>
                        <div class="flex items-start justify-between">
                            ...
                            <Button icon="pi pi-envelope" text rounded size="small"
                                :severity="renewal.is_expired ? 'danger' : 'warning'" v-tooltip="'Relancer'" />
                        </div>
                    </template>
                </Card>
            </div>
            <div v-else class="bg-gray-50 rounded-lg p-8 text-center border border-dashed border-gray-300">
                <font-awesome-icon icon="check-circle" class="text-green-500 text-3xl mb-3" />
                <p class="text-gray-600 font-medium">Aucun renouvellement à prévoir pour les 3 prochains mois.</p>
...
const renewalsNeeded = computed(() => {
    ...
    return renewals.sort((a, b) => new Date(a.expiration_date).getTime() - new Date(b.expiration_date).getTime())
})
