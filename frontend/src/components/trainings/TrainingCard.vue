<template>
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-all duration-200 cursor-pointer h-full flex flex-col relative overflow-hidden group"
        @click="$emit('click')">
        <!-- Left Border for Category Color -->
        <div class="absolute left-0 top-0 bottom-0 w-1.5"
            :style="{ backgroundColor: training.category?.color || '#cbd5e1' }"></div>

        <div class="p-5 pl-7 flex-1 flex flex-col gap-4">
            <!-- Header: Category -->
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2 text-sm text-gray-600">
                    <font-awesome-icon :icon="['fas', training.category?.icon || 'book']"
