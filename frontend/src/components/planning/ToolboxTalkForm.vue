<template>
    <div class="grid grid-cols-1 gap-4">
        <div class="field">
            ...
            <DatePicker id="date" v-model="form.date" dateFormat="dd/mm/yy" class="w-full"
        ...
        <div class="field">
            ...
            <InputText id="topic" v-model="form.topic" class="w-full" :class="{ 'p-invalid': errors.topic }" />
        ...
        <div class="field">
            ...
            <InputText id="location" v-model="form.location" class="w-full" :class="{ 'p-invalid': errors.location }" />
            <small v-if="errors.location" class="text-red-500">{{ errors.location }}</small>
        </div>

        <div class="field">
            <label for="instructor" class="block text-sm font-medium text-gray-700 mb-1">Animateur *</label>
            <Select id="instructor" v-model="form.instructor_id" :options="users" optionLabel="fullname"
                optionValue="id" placeholder="Sélectionner un animateur" class="w-full"
                :class="{ 'p-invalid': errors.instructor_id }" filter />
            <small v-if="errors.instructor_id" class="text-red-500">{{ errors.instructor_id }}</small>
        </div>

        <div class="field">
            <label for="attendees" class="block text-sm font-medium text-gray-700 mb-1">Participants</label>
            <MultiSelect id="attendees" v-model="form.attendees" :options="users" optionLabel="fullname"
                optionValue="id" placeholder="Sélectionner les participants" class="w-full" display="chip" filter />
        </div>
...
const users = computed(() => userStore.users.map(u => ({
    ...u,
...
const save = async () => {
    ...
    try {
        if (props.talk) {
            ...
            toast.add({ severity: 'success', summary: 'Succès', detail: '1/4h Sécurité mis à jour', life: 3000 })
        } else {
            await store.createTalk(data as any)
            toast.add({ severity: 'success', summary: 'Succès', detail: '1/4h Sécurité créé', life: 3000 })
        }
        emit('save')
    } catch (e) {
        console.error(e)
